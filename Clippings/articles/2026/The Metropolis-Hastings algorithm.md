...
source: https://blog.djnavarro.net/posts/2023-04-12_metropolis-hastings/
author:
  - "[[Danielle Navarro]]"
published:
created: 2026-05-29
description: Metropolis-Hastings MCMC primer
tags:
  - clippings
---
Here’s a quick primer on the [Metropolis-Hastings algorithm](https://en.wikipedia.org/wiki/Metropolis%E2%80%93Hastings_algorithm). The target audience for this is someone who has a little bit of probability theory, can write code in R (or similar), but doesn’t have any background in Markov chain Monte Carlo methods. It doesn’t dive deep into the mathematics – i.e., you won’t find any discussions of [detailed balance](https://en.wikipedia.org/wiki/Detailed_balance), [ergodicity](https://en.wikipedia.org/wiki/Ergodicity), and other things a statistics class would cover – but it does try to go deep enough to give a beginner an understanding of what the algorithm is doing.

## The problem

The Metropolis-Hastings algorithm is perhaps the most popular example of a [Markov chain Monte Carlo](https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo) (MCMC) method in statistics. The basic problem that it solves is to provide a method for sampling from some arbitrary probability distribution, which I’ll denote $p \left(\right. x \left.\right)$.[^3] The idea is that in many cases, you know how to write out the equation for the probability $p \left(\right. x \left.\right)$, but you don’t know how to generate a random number from this distribution, $x sim p \left(\right. x \left.\right)$. This is the situation where MCMC is handy. For example, suppose I’ve become interested – for reasons known but to the gods – in the probability distribution shown below:

![](https://blog.djnavarro.net/posts/2023-04-12_metropolis-hastings/index_files/figure-html/unnamed-chunk-1-1.png)

The probability density function [^4] $p \left(\right. x \left.\right)$ for this distribution is given by the following equation:

$$
p \left(\right. x \left.\right) = \frac{exp ⁡ \left(\right. - x^{2} \left.\right) \left(\right. 2 + sin ⁡ \left(\right. 5 x \left.\right) + sin ⁡ \left(\right. 2 x \left.\right) \left.\right)}{\int_{- \infty}^{\infty} exp ⁡ \left(\right. - u^{2} \left.\right) \left(\right. 2 + sin ⁡ \left(\right. 5 u \left.\right) + sin ⁡ \left(\right. 2 u \left.\right) \left.\right) d u}
$$
...
$$
p \left(\right. x \left.\right) \propto exp ⁡ \left(\right. - x^{2} \left.\right) \left(\right. 2 + sin ⁡ \left(\right. 5 x \left.\right) + sin ⁡ \left(\right. 2 x \left.\right) \left.\right)
$$
 How can I generate samples from this distribution?

## The Metropolis-Hastings algorithm

The basic idea behind MCMC is very simple. The idea is to define a [Markov chain](https://en.wikipedia.org/wiki/Markov_chain) [^6] over possible $x$ values, in such a way that the stationary distribution of the Markov chain is in fact $p \left(\right. x \left.\right)$. That is, what we’re going to do is use a Markov chain to generate a sequence of $x$ values, denoted $\left(\right. x_{0} , x_{1} , x_{2} , \ldots , x_{n} \left.\right)$, in such a way that as $n \rightarrow \infty$, we can guarantee that $x_{n} sim p \left(\right. x \left.\right)$. There are many different ways of setting up a Markov chain that has this property. The Metropolis-Hastings algorithm is one of these.
...
The first stage is to generate a *candidate*, which we’ll denote $x^{*}$. The value of $x^{*}$ is generated from the proposal distribution that we already know how to sample from. We denote this proposal distribution $q \left(\right. x^{*} \left|\right. x_{n} \left.\right)$. Notice that the distribution we sample from depends on the current state of the Markov chain, $x_{n}$. There are some technical constraints on what you can use as a proposal distribution, but for the most part it can be anything you like.[^7] A very typical way to do this is to use a normal distribution centered on the current state $x_{n}$. More formally, we write this as:

$$
x^{*} \left|\right. x_{n} sim \text{Normal} \left(\right. x_{n} , \sigma^{2} \left.\right)
$$
 for some standard deviation $\sigma$ that we select in advance (more on this later!)

### The accept-reject step

The second stage is the accept-reject step. Firstly, what you need to do is calculate the *acceptance probability*, denoted $A \left(\right. x_{n} \rightarrow x_{*} \left.\right)$, which is given by:

$$
A \left(\right. x_{n} \rightarrow x_{*} \left.\right) = min \left(\right. 1 , \frac{p \left(\right. x^{*} \left.\right)}{p \left(\right. x^{n} \left.\right)} \times \frac{q \left(\right. x_{n} \left|\right. x^{*} \left.\right)}{q \left(\right. x^{*} \left|\right. x_{n} \left.\right)} \left.\right)
$$
 There are two things to pay attention to here. Firstly, notice that the ratio $p \left(\right. x^{*} \left.\right) / p \left(\right. x^{n} \left.\right)$ doesn’t depend on the normalising constant for the distribution. Or, to put it in a more helpful way, that integral in the first equation is completely irrelevant and we can ignore it. As a consequence, for our toy problem we can write this:

$$
\frac{p \left(\right. x^{*} \left.\right)}{p \left(\right. x^{n} \left.\right)} = \frac{exp ⁡ \left(\right. - x^{*}^{2} \left.\right) \left(\right. 2 + sin ⁡ \left(\right. 5 x^{*} \left.\right) + sin ⁡ \left(\right. 2 x^{*} \left.\right) \left.\right)}{exp ⁡ \left(\right. - x_{n}^{2} \left.\right) \left(\right. 2 + sin ⁡ \left(\right. 5 x_{n} \left.\right) + sin ⁡ \left(\right. 2 x_{n} \left.\right) \left.\right)}
$$
...
The second thing to pay attention to is the behaviour of the other term, $q \left(\right. x_{n} \left|\right. x^{*} \left.\right) / q \left(\right. x^{*} \left|\right. x_{n} \left.\right)$. What this term does is correct for any biases that the proposal distribution might induce. In this expression, the denominator $q \left(\right. x^{*} \left|\right. x_{n} \left.\right)$ describes the probability with which you’d choose $x^{*}$ as the candidate if the current state of the Markov chain is $x_{n}$. The numerator, however, describes the probability of a transition that goes the other way: that is, if the current state had actually been $x^{*}$, what is the probability that you would have generated $x^{n}$ as the candidate value? If the proposal distribution is symmetric, then these two probabilities will turn out to be equal. For example, if the proposal distribution is normal, then:

$$
q \left(\right. x^{*} \left|\right. x_{n} \left.\right) & = & \frac{1}{\sqrt{2 \pi} \sigma} exp ⁡ \left(\right. - \frac{1}{2 \sigma^{2}} \left(\left(\right. x_{n} - x^{*} \left.\right)\right)^{2} \left.\right) \\ q \left(\right. x_{n} \left|\right. x^{*} \left.\right) & = & \frac{1}{\sqrt{2 \pi} \sigma} exp ⁡ \left(\right. - \frac{1}{2 \sigma^{2}} \left(\left(\right. x^{*} - x_{n} \left.\right)\right)^{2} \left.\right)
$$

Clearly, $q \left(\right. x^{*} \left|\right. x_{n} \left.\right) = q \left(\right. x_{n} \left|\right. x^{*} \left.\right)$ for all choices of $x_{n}$ and $x^{*}$, and as a consequence the ratio $q \left(\right. x_{n} \left|\right. x^{*} \left.\right) / q \left(\right. x^{*} \left|\right. x_{n} \left.\right)$ is always 1 in this case.
...
Okay. Having proposed the candidate $x^{*}$ and calculated the acceptance probability, $A \left(\right. x_{n} \rightarrow x^{*} \left.\right)$, we now either decide to “accept” the candidate and set $x_{n + 1} = x^{*}$ or we “reject” the candidate and set $x_{n + 1} = x_{n}$. To make this decision, we generate a uniformly distributed random number between 0 and 1, denoted $u$. Then:

$$
x_{n + 1} = \left{\right. x^{*} & \text{if } u \leq A \left(\right. x_{n} \rightarrow x^{*} \left.\right) \\ x_{n} & \text{otherwise}
$$
...
- `burnin` is the number of iterations we run the sampler for before recording results [^8]
...
![](https://blog.djnavarro.net/posts/2023-04-12_metropolis-hastings/index_files/figure-html/unnamed-chunk-6-1.png)
...
![](https://blog.djnavarro.net/posts/2023-04-12_metropolis-hastings/index_files/figure-html/unnamed-chunk-7-1.png)
...
![](https://blog.djnavarro.net/posts/2023-04-12_metropolis-hastings/index_files/figure-html/unnamed-chunk-8-1.png)
...
![](https://blog.djnavarro.net/posts/2023-04-12_metropolis-hastings/index_files/figure-html/unnamed-chunk-9-1.png)

Formally speaking, the thing we’re trying to do here by increasing `lag` is reduce the [autocorrelation](https://en.wikipedia.org/wiki/Autocorrelation) between successive samples in our chain. In an ideal world we want our sampled values to be *independent* samples from the target distribution $p \left(\right. x \left.\right)$. The more our samples are correlated with each other, the more potential there is for the histogram of sampled values to depart systematically from the target distribution. Introducing a lag between successive samples is a simple way to achieve this.
...
Navarro, Danielle. 2023. “The Metropolis-Hastings Algorithm.” April 12, 2023. [https://blog.djnavarro.net/posts/2023-04-12\_metropolis-hastings/](https://blog.djnavarro.net/posts/2023-04-12_metropolis-hastings/).

[^1]: Bizarrely, this actually happens to me a lot. It’s totally surreal.

[^2]: Another surreal experience that I’ve had quite a bit lately is getting rejected from data science jobs because I don’t have a computer science degree and my qualifications are technically in psychology. Apparently I’m considered skilled enough to *teach* computational statistics to university computer science students, but still considered less skilled at those tools than the students that I taught? I mean, it’s either that or tech company recruiters don’t actually read the résumés that they get sent… but that couldn’t possibly be true, right? Efficient market hypothesis and all that…

[^3]: Okay. So, I just know that a serious statistician will read this at some point, and – from bitter experience – I’ve learned that when a lady person writes about mathematics and is unclear on even the smallest thing her credibility is immediately subject to question. So, in an attempt to forestall any misapprehensions, I most certainly understand the difference between a density function and a distribution function, and equally I know that it is conventional to use lower case to refer to densities and upper case to refer to distributions and probability mass functions. However, I wrote this note for undergraduate computer science students, and accordingly I chose to keep my notation simple, even if it is a little imprecise.

[^4]: Some asides: my experience teaching this class is that it’s quite common for people new to statistics to struggle with the concept of [probability density](https://en.wikipedia.org/wiki/Probability_density_function). It’s not super important for the purposes of this post, and to a first approximation it’s totally okay to think of $p \left(\right. x \left.\right)$ as “the probability of observing $x$ (sort of)”. It is of course the case that the difference between a pdf, a pmf, and a cdf matters when you start doing mathematical statistics, but I would never be so cruel as to inflict that stuff on my undergraduate computer science students! Not once did refer to sigma algebras in my undergrad teaching, honest. Though, looking back, I did once inflict Kolmogorov complexity on a class. It was… not a good decision.

[^5]: It’s worth mentioning that, at this point in the class my students had all been shown several examples of the [curse of dimensionality](https://en.wikipedia.org/wiki/Curse_of_dimensionality) problem. So even though yes, of course it’s easy to solve this problem in one-dimension, they were all acutely aware that using brute force numerical integration methods to compute the denominator isn’t going to work we started trying to do something more realistic than my toy example.

[^6]: At this point in the class students had most certainly encountered Markov chains!

[^7]: That said, it’s worth mentioning one important property of the proposal distribution: the sampler won’t work if there are some values of $x$ that never get proposed!

[^8]: The “burn in” period also goes by the name “warm up” period.

[^9]: This is usually referred to as a “trace plot”. Eyeballing the trace plot is a useful thing to do when examining the behaviour of your MCMC algorithm, but beware… it is a very crude method. The devil is very much in the details here.

[^10]: At this point she asks herself… does she want to talk about methods for automatically choosing a good proposal? No. No she does not.

[^11]: This is often referred to as the “thinning” rate.